...
if [ $? -eq 0 ]; then
  ...
  NVIDIA_DOCKER=`sudo apt list --installed | grep -c nvidia-docker2`
  echo "TEST"$NVIDIA_DOCKER
...
ROUTE_TABLE_PATCHED=`sudo ip route show table local | grep -c $SAGEMAKER_INTERFACE`
